...
def reconstruct_individual(row):
    architecture = eval(row['architecture'])
    initial_seed = int(row['seed'])
    heritage = eval(row['heritage'])
    
    individual = Individual(architecture, initial_seed, row['name'])
    individual.mutation_history = heritage
    
    return individual

def run_trials(individuals, num_trials, environment):
    headless = False
    for individual in individuals:
        ...
        for trial in range(num_trials):
            ...
            if trial % 2:
                emitter_y = np.random.randint(-300, -25)
            else:
                emitter_y = np.random.randint(25, 300)
...
def main():
    ...
    individuals = [reconstruct_individual(row) for _, row in selected_data.iterrows()]
    
    run_trials(individuals, num_trials, environment)
